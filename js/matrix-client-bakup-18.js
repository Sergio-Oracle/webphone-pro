// matrix-client.js — SENDT v18.7
// ✅ Fixes v18.7 :
//   - Fix sonnerie : _processedCallIds Set + délai 3s + filtre âge strict 45s
//   - Présence offline : lastActiveAgo + getLastSeenText()
//   - Typing : emit voice-recording-started/stopped en plus du typing
//   - View once : lecture unique, expose isViewOnce dans les messages parsés
...
class MatrixManager {
    constructor() {
        ...
        this._presenceMap = {};
        // ✅ Fix sonnerie : Set de callIds déjà traités
        ...
        await new Promise(resolve => {
            const onSync = (state) => {
                if (state === 'PREPARED' || state === 'SYNCING') {
                    ...
                    // ✅ 3 secondes pour laisser le SDK traiter les vieux événements
                    setTimeout(() => {
    ...
    _parseEventToMessage(event) {
        ...
        const isViewOnce = !!(content['org.matrix.msc3930.view_once'] || content['m.once']);
        ...
        return { eventId, senderId, sender: senderId, isOwn, type, message, mxcUrl, filename,
                 fileInfo, audioDuration, geoUri, mimetype, isReply, replyToEventId,
                 ephemeral, timestamp, isViewOnce };
    }
    ...
    _setupMatrixListeners() {
        ...
        this.client.on('User.presence', (event, user) => {
            ...
            this._presenceMap[user.userId] = {
                ...
                lastSeenTs: (user.lastActiveAgo != null) ? (now - (user.lastActiveAgo || 0)) : null,
            ...
            if (prev !== user.presence) {
                window.dispatchEvent(new CustomEvent('presence-changed', {
                    detail: {
                        ...
                        lastSeenTs: this._presenceMap[user.userId].lastSeenTs
                    }
                }));
            }
        });

        // ✅ Accusés de lecture (read receipts) — double méthode pour compatibilité
        this.client.on('Room.receipt', (event, room) => {
            if (!room) return;
            const content = event.getContent();

            // Méthode 1 : parser le contenu de l'événement m.receipt
            ...
            // Méthode 2 : émettre aussi via getEventReadUpTo pour robustesse
            try {
                const members = room.getJoinedMembers() || [];
                for (const member of members) {
                    if (member.userId === this.userId) continue;
                    const readEventId = room.getEventReadUpTo?.(member.userId);
                    if (readEventId) {
                        window.dispatchEvent(new CustomEvent('read-receipt-received', {
                            detail: { roomId: room.roomId, eventId: readEventId, userId: member.userId, ts: Date.now() }
                        }));
                    }
                }
            } catch(e) {}
        ...
        // ✅ Typing avec indicateur vocal
        this.client.on('RoomState.events', (event) => {
            if (event.getType() !== 'm.typing') return;
            const roomId = event.getRoomId();
            const userIds = event.getContent()?.user_ids || [];
            const otherTyping = userIds
                .filter(u => u !== this.userId)
                .map(u => { const room = this.client.getRoom(roomId); return room?.getMember(u)?.name || u.split(':')[0].substring(1); });
    ...
    _handleCallInviteEvent(event, room) {
        ...
        // ✅ Triple protection anti-sonnerie fantôme
        // 1. Ignorer si sync initial pas terminé
        if (!this._initialSyncComplete) {
            console.log('[Matrix] 📞 Appel ignoré — sync initial en cours:', content.call_id);
            this._processedCallIds.add(content.call_id);
            return;
        }

        // 2. Ignorer si cet appel a déjà été traité
        if (this._processedCallIds.has(content.call_id)) {
            console.log('[Matrix] 📞 Appel ignoré — déjà traité:', content.call_id);
            return;
        }

        // 3. Ignorer si l'événement est trop ancien (45 secondes)
        const age = Date.now() - event.getTs();
        if (age > 45000) {
            console.log('[Matrix] 📞 Appel ignoré — trop ancien:', Math.round(age/1000) + 's');
            this._processedCallIds.add(content.call_id);
            return;
        }
    ...
    _handleCallHangupEvent(event) {
        if (event.getSender() === this.userId) return;
        const content = event.getContent();
        if (content?.call_id) this._processedCallIds.add(content.call_id);
        this._stopRinging();
    ...
    getUserPresence(userId) {
        return this._presenceMap[userId] || { presence: 'offline', lastSeenTs: null };
    }

    // ✅ Texte "vu il y a Xmin" ou "En ligne"
    getLastSeenText(userId) {
        const p = this._presenceMap[userId];
        if (!p) return 'Hors ligne';
        if (p.presence === 'online' || p.currentlyActive) return 'En ligne';
        if (!p.lastSeenTs) return 'Hors ligne';

        const diff = Date.now() - p.lastSeenTs;
        ...
        return 'vu le ' + new Date(p.lastSeenTs).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short' });
    }
    ...
    _detectRoomType(room) {
        ...
        if (members.length <= 2) {
            ...
            if (!name || (other && (name === other.name || name === other.userId || name.includes(other.userId.split(':')[0])))) return 'dm';
        }
    ...
    _buildRoomInfo(room, roomType) {
        ...
        for (let i = timeline.length - 1; i >= 0; i--) {
            ...
            else if (parsed.type === 'image') lastMessage = '📷 Photo';
    ...
    // ✅ Récupérer les receipts existants d'une room (compatible multi-versions SDK)
    getRoomReadReceipts(roomId) {
        ...
        try {
            ...
            // Méthode 1 : getEventReadUpTo par membre (compatible toutes versions)
            const members = room.getJoinedMembers() || [];
            for (const member of members) {
                if (member.userId === this.userId) continue;
                try {
                    // getEventReadUpTo retourne l'eventId jusqu'auquel ce membre a lu
                    const readEventId = room.getEventReadUpTo?.(member.userId);
                    if (readEventId) {
                        if (!receipts[readEventId]) receipts[readEventId] = [];
                        if (!receipts[readEventId].includes(member.userId)) {
                            receipts[readEventId].push(member.userId);
                        }
                    }
                } catch(e) {}
            }

            // Méthode 2 : getReceiptsForEvent si disponible (SDK récent)
            if (Object.keys(receipts).length === 0) {
                ...
                for (const event of events) {
                    ...
                    if (!eventId) continue;
                    try {
                        const eventReceipts = room.getReceiptsForEvent?.(event) || [];
                        if (eventReceipts.length > 0) {
                            const otherUsers = eventReceipts
                                .filter(r => r.userId && r.userId !== this.userId)
                                .map(r => r.userId);
                            if (otherUsers.length > 0) receipts[eventId] = otherUsers;
                        }
                    } catch(e) {}
                }
            }
    ...
    // ✅ Récupérer les médias partagés d'une room (pour info contact)
    getRoomSharedMedia(roomId, limit = 20) {
        if (!this.client) return [];
        try {
            const room = this.client.getRoom(roomId);
            if (!room) return [];
            const events = room.getLiveTimeline().getEvents();
            const media = [];
            for (let i = events.length - 1; i >= 0 && media.length < limit; i--) {
                const ev = events[i];
                if (ev.getType() !== 'm.room.message') continue;
                const content = ev.getContent();
                if (['m.image', 'm.video'].includes(content.msgtype) && content.url) {
                    media.push({
                        type: content.msgtype === 'm.image' ? 'image' : 'video',
                        mxcUrl: content.url,
    ...
    async getOrCreateRoomForUser(userId) {
        ...
        const resp = await this.client.createRoom({
            preset: 'trusted_private_chat', invite: [userId], is_direct: true,
            initial_state: [{ type: 'm.room.guest_access', state_key: '', content: { guest_access: 'forbidden' } }]
        });
        const roomId = resp.room_id;
        try {
    ...
    async logout() {
        ...
        this._initialSyncComplete = false; this._processedCallIds.clear();
    }
}

const matrixManager = new MatrixManager();
console.log('✅ matrix-client.js v18.7 — Fix sonnerie triple protection, last seen, view once, shared media');
