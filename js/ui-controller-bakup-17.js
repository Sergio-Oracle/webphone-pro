...
class UIController {
    ...
    // ✅ Fix 2 : Injecter les styles de présence une seule fois
    ...
    setupEventListeners() {
        ...
        window.addEventListener('presence-changed', e => {
            const { userId, presence } = e.detail;
            ...
            if (this.currentContact?.userId === userId && !this.currentContact.isGroup) {
                ...
                if (idEl) {
                    const label = presence === 'online' ? '🟢 En ligne' : 'Hors ligne';
                    if (!idEl.textContent.includes('@')) idEl.textContent = label;
                }
            }
        });

        // ✅ Fix 1 : Accusés de lecture
        window.addEventListener('read-receipt-received', e => {
            ...
            // Mettre à jour seulement le tick concerné si possible, sinon re-render
    ...
    // ✅ Fix 1 : Mettre à jour le tick d'un message spécifique
    _updateTickForEvent(roomId, eventId) {
        const tickEl = document.querySelector(`[data-event-id="${eventId}"] .msg-ticks`);
        if (tickEl) {
            tickEl.classList.add('read');
            tickEl.innerHTML = '<i class="fas fa-check-double" style="color:#53bdeb"></i>';
            return;
        }
        // Fallback : re-render complet
        if (this.currentContact?.roomId === roomId) this.renderChatMessages();
    }
    ...
    _renderTypingIndicator() {
        ...
        if (!users.length) { el.classList.remove('show'); return; }
        const text = users.length === 1 ? `${users[0]} écrit...` : `${users.join(', ')} écrivent...`;
        el.innerHTML = `<div class="typing-content"><div class="typing-dots"><span></span><span></span><span></span></div><span class="typing-text">${text}</span></div>`;
    ...
    async startVoiceRecording() {
        ...
        try {
            ...
            this.mediaRecorder.start(); this._showRecUI();
    ...
    renderContacts() {
        ...
        if (allContacts.length) {
            ...
            html += allContacts.map(ct => {
                ...
                return `<div class="contact-item" onclick="uiController.selectContact('${u}')">
    ...
    async selectContact(userId) {
        ...
        document.getElementById('group-mgmt-bar')?.remove();
        const chatHeader = document.querySelector('#contact-view .chat-header');
        if (chatHeader) { chatHeader.style.cursor = ''; chatHeader.title = ''; chatHeader.onclick = null; }
        const cv = document.getElementById('contact-view');
        if (cv) {
            cv.classList.remove('hidden');
            const nameEl = document.getElementById('selected-contact-name');
            ...
            if (idEl) {
                // ✅ Fix 2 : afficher la présence dans le sous-titre
                const presence = this._presenceMap[userId];
                idEl.textContent = presence === 'online' ? '🟢 En ligne' : contact.userId;
            }
        }
        this.cancelReply();
        await this.loadChatHistory();
    ...
    async loadChatHistory() {
        ...
        // ✅ Fix 1 : Charger les receipts existants
        ...
        if (Object.keys(receipts).length > 0) {
            ...
            Object.entries(receipts).forEach(([eventId, userIds]) => {
                this._readReceipts[this.currentContact.roomId][eventId] = new Set(userIds);
            });
        }
    ...
    renderChatMessages() {
        ...
        msgs.forEach((msg, i) => {
            ...
            // ✅ Fix 1 : Déterminer la couleur des ticks (gris = envoyé, bleu = lu)
            let ticks = '';
            if (msg.isOwn) {
                const isRead = this._isMessageRead(this.currentContact.roomId, msg.eventId);
                ticks = `<span class="msg-ticks ${isRead ? 'read' : 'delivered'}"><i class="fas fa-check-double" style="color:${isRead ? '#53bdeb' : '#8696A0'}"></i></span>`;
            }
    ...
    async sendChatMessage() {
        ...
        await matrixManager.sendMessage(this.currentContact.roomId, msg);
    }
    ...
    // ✅ Fix 6 : Historique des appels — design WhatsApp-like complet
    renderCallHistory() {
        const container = document.getElementById('call-history'); if (!container) return;
        const history = callHistoryManager.getHistory();
        if (!history?.length) {
            container.innerHTML = '<div class="empty-state"><i class="fas fa-phone-slash" style="font-size:2rem;margin-bottom:12px"></i><p>Aucun appel récent</p><span style="font-size:.82rem;color:var(--text-muted)">Vos appels apparaîtront ici</span></div>';
            return;
        }
        ...
        if (!document.getElementById('call-history-styles')) {
            ...
            s.textContent = `
                ...
                .call-history-item:hover { background: var(--bg-secondary, rgba(255,255,255,.04)); }
                .call-history-avatar {
                ...
                .call-history-meta .incoming-ok { color: var(--sn-green, #25D366); }
                .call-history-meta .outgoing-ok { color: #53bdeb; }
                .call-history-actions { display: flex; gap: 4px; flex-shrink: 0; }
                .call-history-btn {
                    width: 36px; height: 36px; border-radius: 50%; border: none;
                    background: rgba(37,211,102,.12); color: var(--sn-green, #25D366);
                    cursor: pointer; display: flex; align-items: center; justify-content: center;
                    font-size: .85rem; transition: background .15s;
                }
                .call-history-btn:hover { background: rgba(37,211,102,.25); }
                .call-history-btn.video { background: rgba(79,172,254,.12); color: #4facfe; }
                .call-history-btn.video:hover { background: rgba(79,172,254,.25); }
                .call-count-badge {
        ...
        // Grouper les appels consécutifs avec le même contact
        const grouped = [];
        let current = null;
        for (const call of history) {
            if (current && current.displayName === call.displayName && current.status === call.status && current.type === call.type && (call.timestamp - current._lastTs) < 3600000) {
                current.count++;
                current._lastTs = call.timestamp;
            } else {
                if (current) grouped.push(current);
                current = { ...call, count: 1, _lastTs: call.timestamp };
            }
        }
        if (current) grouped.push(current);

        const statusConfig = {
            completed: { label: 'Terminé', icon: 'fa-arrow-down-left', className: 'incoming-ok' },
            missed:    { label: 'Manqué',  icon: 'fa-phone-missed',     className: 'missed' },
            no_answer: { label: 'Sans réponse', icon: 'fa-phone-slash', className: 'missed' },
            declined:  { label: 'Refusé',  icon: 'fa-phone-slash',      className: 'missed' },
        };
        ...
        grouped.forEach(call => {
            ...
            // Direction + statut de l'appel
            const dirIcon = isOut ? 'fa-arrow-up-right' : (isMissed ? 'fa-phone-slash' : 'fa-arrow-down-left');
            const statusClass = isMissed ? 'missed' : (isOut ? 'outgoing-ok' : 'incoming-ok');
            const statusLabel = isMissed ? (isOut ? 'Non répondu' : cfg.label) : (isOut ? 'Émis' : 'Reçu');
            const durStr = call.duration ? ` · ${formatCallDuration ? formatCallDuration(call.duration) : Math.floor(call.duration / 60) + ':' + String(call.duration % 60).padStart(2,'0')}` : '';
            const timeStr  = typeof formatDate === 'function' ? formatDate(call.timestamp) : new Date(call.timestamp).toLocaleDateString('fr-FR');
            const countBadge = call.count > 1 ? `<span class="call-count-badge">${call.count}</span>` : '';
            const userId = call.userId || '';

            html += `<div class="call-history-item" onclick="uiController._callHistoryItemClick('${this.sanitize(userId)}')">
                <div class="call-history-avatar" style="background:${bgColor}">${initial}</div>
                <div class="call-history-info">
                    <div class="call-history-name">${name}</div>
                    <div class="call-history-meta">
                        <i class="fas ${dirIcon} ${statusClass}" style="font-size:.75rem"></i>
                        <i class="fas ${typeIcon}" style="font-size:.72rem"></i>
                        <span class="${statusClass}">${statusLabel}${durStr}</span>
                        ${countBadge}
                        <span style="margin-left:auto">${timeStr}</span>
                    </div>
                </div>
                <div class="call-history-actions">
                    <button class="call-history-btn" onclick="event.stopPropagation();uiController._callHistoryCall('${this.sanitize(userId)}', false)" title="Appel audio">
                        <i class="fas fa-phone"></i>
                    </button>
                    <button class="call-history-btn video" onclick="event.stopPropagation();uiController._callHistoryCall('${this.sanitize(userId)}', true)" title="Appel vidéo">
