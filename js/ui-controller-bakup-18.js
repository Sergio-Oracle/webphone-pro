// ui-controller.js — SENDT v15.3
// ✅ Fixes v15.3 :
//   - Fix 1 : Accusés de lecture — double tick bleu quand message lu
//   - Fix 2 : Présence — point vert/gris dans la liste des contacts
//   - Fix 4 : Ajout contact — refresh immédiat sans reconnexion
//   - Fix 6 : Historique appels redesign WhatsApp-like
...
class UIController {
    constructor() {
        ...
        this._readReceipts = {};
        // ✅ Fix 2 : Présence {userId: 'online'|'offline'|'unavailable'}
        this._presenceMap = {};
    ...
    init() { this.setupEventListeners(); this._injectPresenceStyles(); }
    ...
    async _openViewOnce(eventId, type, mxcUrl) {
        ...
        if (type === 'image') {
            await this.showImageFullscreen(mxcUrl);
        } else {
            await this.downloadFile(mxcUrl, 'media_vue_unique');
        }
        // Re-render pour montrer "déjà ouvert"
    ...
    // ✅ Fix 2 : Injecter les styles de présence + typing WhatsApp + contact info
    _injectPresenceStyles() {
        ...
        s.textContent = `
            ...
            .msg-ticks.sending i   { color: #8696A0; font-size: .7rem; }  /* ✓ envoi en cours */
            .msg-ticks.delivered i { color: #8696A0; }                    /* ✓✓ envoyé */
            .msg-ticks.read i      { color: #53bdeb; }                    /* ✓✓ lu (bleu) */
            ...
            .view-once-opened { opacity: .5; filter: blur(2px); pointer-events: none; }
    ...
    setupEventListeners() {
        ...
        window.addEventListener('presence-changed', e => {
            const { userId, presence, lastSeenTs } = e.detail;
            this._presenceMap[userId] = presence;
            this._updatePresenceDot(userId, presence);
            // Mettre à jour le sous-titre si c'est le contact ouvert
            if (this.currentContact?.userId === userId && !this.currentContact.isGroup) {
                const idEl = document.getElementById('selected-contact-id');
                if (idEl) {
                    idEl.textContent = presence === 'online'
                        ? '🟢 En ligne'
                        : matrixManager.getLastSeenText(userId);
                }
            }
        });
        ...
        window.addEventListener('read-receipt-received', e => {
            ...
            if (this.currentContact?.roomId === roomId) {
                ...
                // Mettre à jour le tick ciblé (sans re-render complet)
        ...
        window.addEventListener('member-joined', e => {
            ...
            // Vérifier si c'est un contact qu'on a ajouté récemment
            const existingContact = this.contacts.find(c => c.userId === userId);
            if (!existingContact) {
                // Recharger pour obtenir le nouveau contact
                setTimeout(() => matrixManager.loadRooms(), 500);
            }
        });
    ...
    // ✅ Mettre à jour les ticks de TOUS les messages lus (propagation WhatsApp)
    // Appelé après chaque receipt : met à jour tous les messages own qui sont maintenant "lus"
    _updateTickForEvent(roomId, eventId) {
        ...
        for (const msg of msgs) {
            ...
            // Ce message est lu — mettre à jour son tick dans le DOM
        ...
        // Si aucun élément DOM trouvé (messages pas encore rendus), forcer un re-render
        if (domUpdated === 0) this.renderChatMessages();
    }

    // ✅ Fix 1 : Vérifie si un eventId a été lu par quelqu'un d'autre
    ...
    // ✅ Typing WhatsApp-like : texte, vocal, ou enregistrement
    ...
    // ✅ Appeler depuis startVoiceRecording/stopVoiceRecording pour notifier les contacts
    _notifyVoiceRecordingState(isRecording) {
        // Émettre l'état dans la sidebar pour le contact actuel
    ...
    _renderTypingIndicator() {
        ...
        // Déterminer le texte selon le nombre de personnes
        let icon = '', text = '';
        if (users.length === 1) {
            icon = `<span class="typing-wave"><span></span><span></span><span></span></span>`;
            text = `<strong>${users[0]}</strong> est en train d'écrire`;
        } else if (users.length === 2) {
            icon = `<span class="typing-wave"><span></span><span></span><span></span></span>`;
            text = `<strong>${users[0]}</strong> et <strong>${users[1]}</strong> écrivent`;
        } else {
            icon = `<span class="typing-wave"><span></span><span></span><span></span></span>`;
            text = `<strong>${users.length} personnes</strong> écrivent`;
        }
    ...
    async startVoiceRecording() {
        ...
        try {
            ...
            this.mediaRecorder.onstop = async () => {
                ...
                // Arrêter l'indicateur d'enregistrement dans la liste
            ...
            // ✅ Notifier le serveur qu'on enregistre (typing)
            matrixManager.sendTyping(this.currentContact.roomId, true);
            // Montrer "🎙️ Enregistrement vocal..." dans la liste de contacts
    ...
    // ✅ Met à jour le dernier message affiché dans la liste de contacts
    ...
    // ═══════════════ CONTACTS ═══════════════
    // ✅ Fix 2 : Ajouter les points de présence dans la liste des contacts
    renderContacts() {
        ...
        if (allContacts.length) {
            ...
            html += allContacts.map(ct => {
                ...
                // ✅ Fix 2 : point de présence
                const presence = this._presenceMap[ct.userId] || 'offline';
                const presenceDotHtml = `<div class="presence-dot ${presence === 'online' ? 'online' : 'offline'}" id="presence-${safeId}"></div>`;
    ...
    async _loadContactAvatar(userId, mxcUrl) {
        ...
        if (b) {
            ...
            if (el) {
                // Garder le point de présence
                const presence = this._presenceMap[userId] || 'offline';
                el.innerHTML = `<img src="${b}" class="contact-avatar-img" alt=""><div class="presence-dot ${presence === 'online' ? 'online' : 'offline'}" id="presence-${safeId}"></div>`;
            }
        }
    }
    ...
    async showGroupInfoModal(roomId) {
        ...
        let invitedMembers = [];
        try { invitedMembers = room.getMembersWithMembership?.('invite') || []; } catch(e) {}
        members.sort((a, b) => {
        ...
        const memberItems = members.map(m => {
            ...
            // ✅ Fix 2 : présence dans la liste des membres
            const presence = this._presenceMap[uid] || 'offline';
            const presenceDot = `<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${presence === 'online' ? '#25D366' : '#6a6f74'};margin-left:4px;vertical-align:middle"></span>`;
    ...
    async selectContact(userId) {
        ...
        if (chatHeader) {
            ...
            chatHeader.onclick = (e) => {
                if (!e.target.closest('.icon-btn, .call-btn, button')) this.showContactInfoModal(userId);
            };
        }
        const cv = document.getElementById('contact-view');
        if (cv) {
            ...
            if (idEl) {
                // ✅ Afficher "En ligne" ou "vu il y a Xmin" comme WhatsApp
                const presence = this._presenceMap[userId];
                if (presence === 'online') {
                    idEl.textContent = '🟢 En ligne';
                } else {
                    idEl.textContent = matrixManager.getLastSeenText(userId);
                }
            }
        }
        this.cancelReply();
    ...
    // ✅ Panel info contact (WhatsApp-like)
    async showContactInfoModal(userId) {
        ...
        const lastSeen = matrixManager.getLastSeenText(userId);
        const initial  = contact.displayName.charAt(0).toUpperCase();
        const colors   = ['#25D366','#128C7E','#4facfe','#f093fb','#ffa726','#e74c3c','#9b59b6'];
        const bgColor  = colors[initial.charCodeAt(0) % colors.length];

        // Récupérer les médias partagés
        ...
        const mediaHtml = sharedMedia.length > 0
            ...
            : '<div style="padding:12px 0;color:var(--text-muted);font-size:.82rem">Aucun média partagé</div>';
        ...
        const avatarContent = avatarUrl
            ? `<img src="${avatarUrl}" style="width:96px;height:96px;border-radius:50%;object-fit:cover" alt="">`
            : `<span style="background:${bgColor};width:96px;height:96px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:2.2rem;font-weight:700;color:#fff">${initial}</span>`;

        modal.innerHTML = `
        <div class="modal-content" style="max-width:480px;max-height:90vh;overflow-y:auto;padding:0">
            <!-- En-tête -->
            <div class="contact-info-header">
                <div style="position:relative;display:inline-block;margin-bottom:12px">
                    ${avatarContent}
                    <div class="contact-info-presence" style="background:${isOnline ? '#25D366' : '#6a6f74'};position:absolute;bottom:4px;right:4px;width:16px;height:16px;border-radius:50%;border:3px solid #1F2C34"></div>
                </div>
                <div class="contact-info-name">${this.sanitize(contact.displayName)}</div>
                <div class="contact-info-status">${this.sanitize(isOnline ? '🟢 En ligne' : lastSeen)}</div>
            </div>
            ...
            <div class="contact-info-section">
                <div class="contact-info-row">
                    ...
                    <div><div class="contact-info-label">Identifiant Matrix</div><div class="contact-info-value">${this.sanitize(userId)}</div></div>
                </div>
                <div class="contact-info-row">
                    <i class="fas fa-clock contact-info-icon"></i>
                    <div><div class="contact-info-label">Dernière activité</div><div class="contact-info-value">${this.sanitize(lastSeen)}</div></div>
                </div>
            </div>

            <!-- Médias partagés -->
            <div class="contact-info-section">
                <div style="padding:12px 0 4px;font-size:.72rem;font-weight:600;color:var(--text-muted);text-transform:uppercase">
            ...
            <button onclick="closeModal('contact-info-modal')" style="position:absolute;top:12px;right:12px;background:rgba(255,255,255,.15);border:none;color:#fff;width:28px;height:28px;border-radius:50%;cursor:pointer;font-size:.9rem;display:flex;align-items:center;justify-content:center">
        ...
        if (!avatarUrl && contact.avatarMxc) {
            ...
            if (b) {
                const imgContainer = modal.querySelector('.contact-info-header div[style*="inline-block"] span');
                if (imgContainer) imgContainer.innerHTML = `<img src="${b}" style="width:96px;height:96px;border-radius:50%;object-fit:cover" alt="">`;
            }
        }
    }
    ...
    async loadChatHistory() {
        ...
        if (Object.keys(receipts).length > 0) {
            ...
            // ✅ Propagation : marquer aussi tous les messages PRÉCÉDANT le dernier receipt
            // (comme WhatsApp : si le dernier message est lu, tous les précédents le sont aussi)
            const sortedMsgs = [...msgs].sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
            const receiptEventIds = Object.keys(receipts);
            // Trouver le timestamp du receipt le plus récent
            let latestReadTs = 0;
            for (const eventId of receiptEventIds) {
                const msgWithId = sortedMsgs.find(m => m.eventId === eventId);
                if (msgWithId && (msgWithId.timestamp || 0) > latestReadTs) {
                    latestReadTs = msgWithId.timestamp || 0;
                }
            }
            // Tous les messages own avant ce timestamp sont aussi lus
            if (latestReadTs > 0) {
    ...
    renderChatMessages() {
        ...
        msgs.forEach((msg, i) => {
            ...
            // ✅ Ticks WhatsApp 3 états :
            // ✓  gris = envoi en cours (pas encore de vrai eventId)
            // ✓✓ gris = envoyé/livré (eventId réel)
            // ✓✓ bleu = lu (receipt reçu)
            let ticks = '';
            if (msg.isOwn) {
                ...
                if (!hasRealId) {
                    // Envoi en cours — simple tick gris
    ...
    _renderContent(msg, i) {
        // ✅ Vue unique : afficher un badge spécial si non encore ouvert
        if (msg.isViewOnce) {
            ...
            // Déjà ouvert : afficher normalement mais marqué
        }
    ...
    async sendChatMessage() {
        ...
        // ✅ Ajout optimiste : message visible immédiatement avec tick simple (envoi en cours)
        ...
        // La confirmation serveur remplacera le message via handleIncomingMessage
    }
    ...
    handleIncomingMessage(data) {
        ...
        if (hasRealId) {
            ...
            if (data.isOwn) {
                const echoIdx = msgs.findIndex(m => { if (!m.isOwn || m.type !== data.type) return false; if (this._isRealEventId(m.eventId)) return false; if (Math.abs((m.timestamp || 0) - data.timestamp) > 60000) return false; if (data.type === 'text') return (m.message || '').trim() === (data.message || '').trim(); if (['image', 'voice', 'video', 'file'].includes(data.type)) return m.mxcUrl && data.mxcUrl && m.mxcUrl === data.mxcUrl; return (m.message || '') === (data.message || ''); });
        ...
        if (data.isOwn) {
            const isDup = msgs.some(m => { if (!m.isOwn || m.type !== data.type) return false; if (Math.abs((m.timestamp || 0) - data.timestamp) > 60000) return false; if (data.type === 'text') return (m.message || '').trim() === (data.message || '').trim(); if (['image', 'voice', 'video', 'file'].includes(data.type)) return m.mxcUrl && data.mxcUrl && m.mxcUrl === data.mxcUrl; return false; });
            if (isDup) return;
        } else {
    ...
    _afterMessageAdd(rid, data, isNew) {
        ...
        if (isNew && !data.isOwn) {
            const ct = this.contacts.find(c => c.roomId === rid) || this.groups.find(g => g.roomId === rid);
            if (ct) showToast(`${ct.displayName} : ${data.type === 'text' ? (data.message || '').substring(0, 45) : '📎 Fichier'}`, 'info');
        }
    }
    ...
    async addNewContact() {
        ...
        try {
            ...
            // Vérifier si ce contact existe déjà localement
            const existingContact = this.contacts.find(c => c.userId === uid || c.roomId === rid);
            if (!existingContact) {
                // L'ajouter temporairement en attendant le rechargement
            ...
            // Forcer un rechargement des rooms après un délai
    ...
    // ✅ Fix 6 v2 : Historique appels WhatsApp — temps réel, format exact
    renderCallHistory() {
        const container = document.getElementById('call-history'); if (!container) return;
        // Utiliser callHistoryManager si disponible, sinon matrixManager
        let history = [];
        ...
        // Injecter les styles de l'historique si nécessaire
        if (!document.getElementById('call-history-styles')) {
            ...
            s.textContent = `
                ...
                .call-history-btn {
                    ...
                    background: rgba(37,211,102,.1); color: var(--sn-green,#25D366);
                ...
                .call-history-count {
                    ...
                    border-radius: 10px; padding: 0 6px; font-size: .68rem; height: 16px;
                    margin-left: 2px;
                }
            `;
            document.head.appendChild(s);
        ...
        // Grouper appels consécutifs identiques (même contact, même statut, même type, < 1h)
        const grouped = [];
        let current = null;
        for (const call of history) {
            ...
            if (current && current._key === key && (call.timestamp - current._lastTs) < 3600000) {
                current.count++;
                current._lastTs = call.timestamp;
            } else {
        ...
        grouped.forEach(call => {
            ...
            if (isMissed && !isOut) {
                dirIcon = 'fa-phone-missed'; dirClass = 'missed'; dirLabel = 'Appel manqué';
            } else if (isMissed && isOut) {
                dirIcon = 'fa-phone-missed'; dirClass = 'missed'; dirLabel = 'Sans réponse';
            } else if (isOut) {
                dirIcon = 'fa-arrow-up'; dirClass = 'outgoing'; dirLabel = 'Émis';
            } else {
                dirIcon = 'fa-arrow-down'; dirClass = 'incoming'; dirLabel = 'Reçu';
            }
    ...
    _callHistoryItemClick(userId) {
        // Naviguer vers le contact si possible
        if (!userId) return;
        const contact = this.contacts.find(c => c.userId === userId);
        if (contact) {
            // Changer de tab vers Chats et sélectionner le contact
...
console.log('✅ ui-controller.js v15.3 — Read receipts bleus, présence, contact refresh, historique appels WhatsApp');
