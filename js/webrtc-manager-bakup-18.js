...
class WebRTCManager {
    ...
    // ✅ Grille WhatsApp — tiles remplissent l'écran, une seule rangée par participant
    _refreshGridLayout() {
        const container = document.getElementById('group-video-container');
        if (!container) return;
        const tiles = Array.from(container.querySelectorAll('.group-video-tile'));
        ...
        // Définir la grille selon le nombre de participants (comme WhatsApp)
        let cols, rows;
        if      (n === 1) { cols = 1; rows = 1; }
        else if (n === 2) { cols = 1; rows = 2; }  // 2 tiles empilées verticalement
        else if (n === 3) { cols = 2; rows = 2; }  // 2 haut + 1 bas centré
        ...
        // Cas impairs : dernier tile sur toute la largeur
        if (n === 3) tiles[2].style.gridColumn = '1 / -1';
        if (n === 5) tiles[4].style.gridColumn = '1 / -1';
        if (n === 7) tiles[6].style.gridColumn = '1 / -1';
    }
    ...
    _buildGroupCallScreen() {
        ...
        if (!document.getElementById('lk-group-styles')) {
            ...
            style.textContent = `
                /* ═══ CALL SCREEN : dimensions fixes pour que le grid fonctionne ═══ */
                #call-screen.active {
                ...
                #group-video-container {
                    ...
                    overflow: hidden;
                    /* ✅ CRUCIAL : les cellules s'étirent pour remplir leur cellule */
                    align-content: stretch;
                    justify-content: stretch;
                }

                /* ═══ TILES : NE PAS mettre width/height 100% — le grid le gère ═══ */
                .group-video-tile {
                    position: relative;
                    background: #1a2332;
                    border-radius: 0;
                    ...
                    /* Ces 2 lignes empêchent le débordement dans le grid */
                    min-height: 0;
                    min-width: 0;
                }
                .group-video-tile.speaking {
                    border-color: #25D366;
                    box-shadow: inset 0 0 0 2px rgba(37,211,102,.5);
                }
                ...
                @media (min-width: 600px) {
                    #group-video-container { padding: 60px 0 92px; gap: 4px; }
                    ...
                    .lk-ctrl-hangup { width: 60px !important; height: 60px !important; }
                }

                @media (min-width: 900px) {
                    #group-video-container { padding: 64px 0 96px; gap: 4px; }
                    .group-video-tile { border-radius: 4px; }
                    ...
                    .lk-ctrl-hangup { width: 64px !important; height: 64px !important; }
                ...
                @media (min-width: 1200px) {
                    #group-video-container { padding: 68px 0 100px; gap: 5px; }
